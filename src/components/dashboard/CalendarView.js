...
function WorkloadBar({ week_start, heaviness, assignment_count, pending_count }) {
    ...
    return (
        <div className="flex flex-col items-center gap-0.5" title={`${assignment_count} assignments (${pending_count} pending)`}>
            <div className="w-8 bg-subtle rounded-full overflow-hidden" style={{ height: 40 }}>
                <div className="w-full rounded-full transition-all" style={{ height: `${heaviness}%`, backgroundColor: color, marginTop: `${100 - heaviness}%` }} />
            </div>
            <span className="text-xs text-ink-faint text-center leading-tight">{label}</span>
...
export default function CalendarView() {
    ...
    if (loading) {
        return (
            <div className="card p-6 h-96 flex items-center justify-center">
                <p className="text-ink-faint">Loading calendar…</p>
            </div>
        );
    }
    ...
    return (
        <div className="card p-4">
            ...
            {workload.length > 0 && (
                <div className="mb-4">
                    <p className="text-xs text-ink-faint font-medium mb-2 uppercase tracking-wide">Upcoming workload</p>
